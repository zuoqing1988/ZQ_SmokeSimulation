...
bool ZQ_MergeImage::Go(const ZQ_MergeImageOptions& opt)
{
...
bool ZQ_MergeImage::Go_MergeDirectly(const ZQ_MergeImageOptions& opt)
{
...
bool ZQ_MergeImage::Go_MergeDensity(const ZQ_MergeImageOptions& opt)
{
...
bool ZQ_MergeImage::Go_MergeSourcePatch(const ZQ_MergeImageOptions& opt)
{
...
bool ZQ_MergeImage::Go_ImageBlur(const ZQ_MergeImageOptions& opt)
{
	if(!opt.has_source_file)
	{
		if(opt.display_running_info)
...
bool ZQ_MergeImage::Go_MergeLowHigh(const ZQ_MergeImageOptions& opt)
{
	if(!opt.has_source_file)
	{
		if(opt.display_running_info)
...
void ZQ_MergeImage::MakeMatrix(const Vec2& scale, const float rot_rad, const Vec2& trans, float output_mat[9])
{
	float mat_scale[9] = 
	{
		scale.x, 0, 0,
...
bool ZQ_MergeImage::MergeDirectly(DImage& output, const std::vector<DImage>& inputs,bool display)
{
	int num = inputs.size();
	if(num == 0)
	{
...
bool ZQ_MergeImage::MergeDensity(DImage& output, const std::vector<DImage>& inputs, const std::vector<Vec2>& target_size, const std::vector<float>& rots, const std::vector<Vec2>& trans,  const bool blend_mode, const bool yAxisUp, const bool display)
{
	int num = inputs.size();
...
bool ZQ_MergeImage::_renderToBackground(ZQ_CPURenderer2DWorkspace& renderer, const DImage& source, const Vec2& target_size, const float rot_rad, const Vec2& trans)
{
...
bool ZQ_MergeImage::MergeSourcePatch(DImage& output, const DImage& source, const DImage& patch, const DImage& mask)
{
...
void ZQ_MergeImage::DecomposeByBlur(DImage& low, DImage& high, const DImage& input, const float sigma, const int fsize)
{
	input.GaussianSmoothing(low,sigma,fsize);
	high.Subtract(input,low);
}

bool ZQ_MergeImage::_load(DImage& img, const char* file, const int isColor)
{
...
bool ZQ_MergeImage::_save(const DImage&img, const char* file)
{
	const char* suffix_di2 = ".di2";
	int suffix_di2_len = strlen(suffix_di2);
